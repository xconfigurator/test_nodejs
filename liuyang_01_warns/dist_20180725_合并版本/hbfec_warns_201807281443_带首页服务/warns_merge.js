...
// 设备信息代码表
// const DIC_DEVICE_TYPE = warnsDic.DIC_DEVICE_TYPE // TODO 暂时注释
// 告警级别代码表
// 网管系统内部级别告警级别，与chenchen讨论后确定 20180723 liuyang
// const DIC_WARN_LEVEL = warnsDic.DIC_WARN_LEVEL // TODO 暂时注释
// 设备告警状态(依据：TAU告警库_201806250849.docx)
// const DIC_WARN_DESC_TAU = warnsDic.DIC_WARN_DESC_TAU // TODO 暂时注释
// const DIC_WARN_DESC_CPE = warnsDic.DIC_WARN_DESC_CPE
// const DIC_WARN_DESC_CZT = warnsDic.DIC_WARN_DESC_CZT
// const DIC_WARN_DESC_GDT = warnsDic.DIC_WARN_DESC_GDT
 ...
 */
function processOneDevice (deviceInfo) {
  // 1. 抽取告警信息（每台设备）
  // 其实可以写成 var warnsInfo = extractTheLatestWarnInfoSet(extractWarnsInfo(result[idx]))
  // 但为了方便排错，还是写成下面的形式。简化写法只表明两个函数之间的功能渐进增强关系。
 ...
 */
function deviceTAUCPEExtractor (warnObj) {
  log.debug('deviceTAUCPEExtractor')
  let warnData = []
  // 解析出设备上报的所有告警信息（注：设备可能将同一告警信息上报多次）
  for (var key in warnObj) {
    // 设备差异点1：设备上报信息原始结构可能不同
