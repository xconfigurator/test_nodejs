...
function main () {
  findAll('devices', {}, function (err, result) {
    ...
    for (var idx in result) {
      ...
      if (isDebug && warnsInfo.warnData.length !== 0) console.log('#debug main 清洗前 warnsInfo = ' + JSON.stringify(warnsInfo))
      warnsInfo = extractTheLatestWarnInfoSet(warnsInfo)
      if (isDebug && warnsInfo.warnData.length !== 0) console.log('#debug main 清洗后 warnsInfo = ' + JSON.stringify(warnsInfo))
...
function processWarnsMerge (warnsInfo, warnsInfoFromDB) {
  ...
  if (warnsInfo == null && warnsInfoFromDB != null) {
    // 1. 将warnsInfoFromDB中所有信息归档至warnsHistory
    for (let idxDb in warnsInfoFromDB.warnData) {
      dbSaveWarnItemToWarnsHistory(warnsInfoFromDB.warnData[idxDb], warnsInfoFromDB)
    }
  ...
  for (let idxDevice in warnsInfo.warnData) { // 遍历设备上报信息的每一条记录，看数据库中是否已有相关告警
    ...
    for (let idxDb in warnsInfoFromDB.warnData) {
      if (isWarnsInfoHasSameContent(warnsInfoFromDB.warnData[idxDb], warnsInfo.warnData[idxDevice])) {
        ...
        if (isWarnsInfoHasLatterTime(warnsInfo.warnData[idxDevice], warnsInfoFromDB.warnData[idxDb])) {
          ...
          if (isDebug) console.log('#debug processWarnsMerge 更新 更新后 warnItemFromDB.warnData[idxDb] = ' + JSON.stringify(warnsInfoFromDB.warnData[idxDb]))
          if (isDebug) console.log('#debug processWarnsMerge 更新 更新后 warnItemFromDB.warnData = ' + JSON.stringify(warnsInfoFromDB.warnData))
          if (isDebug) console.log('#debug processWarnsMerge 更新 更新后 warnItemFromDB = ' + JSON.stringify(warnsInfoFromDB))
          update('warns', {'deviceId': warnsInfoFromDB.deviceId}, {$set: {'warnData': warnsInfoFromDB.warnData}}, function (err, result) {
            if (err) throw err
            if (isDebug) console.log('#debug processWarnsMerge 更新 数据库更新返回结果 result = ' + JSON.stringify(result))
          })
          // 2. WebSocket通知
          // TODO
        }
    ...
    if (!flagWarnItemHasTheSameContent) { // 数据库中尚无该告警信息
      ...
      update('warns', {'deviceId': warnsInfoFromDB.deviceId}, {$set: {'warnData': warnsInfoFromDB.warnData}}, function (err, result) {
        if (err) throw err
        if (isDebug) console.log('#debug processWarnsMerge 新增 数据库更新返回结果 result = ' + JSON.stringify(result))
      })
      // 2. WebSocket通知
      // TODO
    }
  ...
  if (isDebug) console.log('#debug processWarnsMerge 第二遍遍历后 warnsInfoFromDB.warnData = \n' + JSON.stringify(warnsInfoFromDB.warnData))
  if (isDebug) console.log('#debug processWarnsMerge 第二遍遍历后 warnDataNew = \n' + JSON.stringify(warnDataNew))

  //  1. 删除告警逻辑
  //  1.1. 将该设备的该条告警信息归档至warnsHistory集合
  ...
  if (warnDataNew == null || warnDataNew.length === 0) {
    ...
    */
    ...
    update('warns', {'deviceId': warnsInfoFromDB.deviceId}, {$set: {'warnData': warnDataNew}}, function (err, result) {
      if (err) throw err
      if (isDebug) console.log('#debug processWarnsMerge 告警集合仍有告警信息 更新至数据库 db result = ' + JSON.stringify(result))
    })
  }
  // 2. WebSocket通知
  // TODO
...
/**
 * 将指定设备的指定告警信息保存至warnsHistory集合
 * @param {Object} warnItem 待保存到warnsHistory的告警项
 * @param {Object} warnsInfo 设备告警信息（勇于获得设备信息）
 * @author liuyang
 * @since 2018/6/28
 */
function dbSaveWarnItemToWarnsHistory(warnItem, warnsInfo) {
  ...
  if (isDebug) console.log('#debug dbSaveWarnItemToWarnsHistory 归档 待归档记录 warnItemHistory = \n' + JSON.stringify(warnItemHistory))

  insertOne('warnsHistory', warnItemHistory, function (err, result) {
    if (err) throw err
    if (isDebug) console.log('#debug dbSaveWarnItemToWarnsHistory 归档 归档至warnsHistory 集合 db result = ' + JSON.stringify(result))
  })
}
...
function dbDeleteWarnInfoFromWarns(warnsInfo) {
  deleteOne('warns', {'deviceId': warnsInfo.deviceId}, function (err, result) {
    if (err) throw err
    if (isDebug) console.log('#debug dbDeleteWarnInfoFromWarns 删除warns中该设备的告警文档 db result = ' + JSON.stringify(result))
  })
}
...
exports.getCurrentDateTimeStr = getCurrentDateTimeStr // TODO
